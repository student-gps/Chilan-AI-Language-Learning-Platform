...
# Minimal dependency shims so the publish script can be imported in this test env.
...
from content_builder.ja.minna_no_nihongo.scripts import run_publish_pipeline as publish_script  # noqa: E402
...
class MinnaNoNihongoPublishScriptTests(unittest.TestCase):
    def test_resolve_stage1_outputs_prefers_output_json_and_respects_reuse_synced(self):
        tmpdir = Path(self._make_tempdir())
        artifact_root = tmpdir / "artifacts"
        out_dir = artifact_root / "output_json" / "zh"
        synced_dir = artifact_root / "synced_json" / "zh"
        out_dir.mkdir(parents=True, exist_ok=True)
        synced_dir.mkdir(parents=True, exist_ok=True)
        output_file = out_dir / "lesson001_data.json"
        synced_file = synced_dir / "lesson002_data.json"
        ...
        with patch.object(publish_script, "default_paths", return_value=types.SimpleNamespace()), patch.object(
            ...
            outputs = publish_script._resolve_stage1_outputs(args)

        self.assertEqual(outputs, [output_file, synced_file])

    def test_validate_outputs_passes_for_valid_japanese_payload(self):
        tmpdir = Path(self._make_tempdir())
        image_path = tmpdir / "slide_001.png"
        audio_path = tmpdir / "lesson001_slide_001_zh.mp3"
        narration_path = tmpdir / "lesson001_narration_zh.mp3"
            ...
            publish_script.json.dumps(
                {
                    ...
                    "target_language": "ja",
                    "lesson_audio_assets": {
                        "items": [{"audio_id": "dialogue_001", "status": "ready"}],
                    },
        ...
        publish_script._validate_outputs([json_path], "zh")

    def test_validate_outputs_raises_on_missing_lesson_audio_items(self):
        tmpdir = Path(self._make_tempdir())
        json_path = tmpdir / "lesson001_data.json"
        json_path.write_text(
            publish_script.json.dumps(
                {
                    "pipeline_id": "minna_no_nihongo",
                    "target_language": "ja",
                    "lesson_audio_assets": {"items": []},
                    "explanation_narration_audio": {"status": "error", "audio_file": ""},
                    "video_render_plan": {"explanation": {"segments": []}},
                    "teaching_slide_deck": {"lang": "zh", "slide_count": 0, "slides": []},
                },
                ensure_ascii=False,
                indent=2,
            ),
    ...
    def test_run_step_raises_on_nonzero_exit(self):
        ...
        self.assertIn("failing-step failed with exit code 2", str(ctx.exception))

    def test_main_reuse_stage1_json_skips_build_and_stops_before_sync(self):
        tmpdir = Path(self._make_tempdir())
        target = tmpdir / "lesson001_data.json"
        target.write_text("{}", encoding="utf-8")

        argv = [
            "run_publish_pipeline.py",
            "--lang",
            "zh",
            "--lesson",
            "1",
        ...
        with patch.object(sys, "argv", argv), patch.object(publish_script, "load_dotenv", return_value=None), patch.object(
            publish_script, "_resolve_stage1_outputs", return_value=[target]
        ...
        self.assertIn("Skipping build_lesson.py", output)
        ...
        self.assertEqual(validate_calls, [([target], "zh")])

    def _make_tempdir(self) -> str:
        import tempfile

        return tempfile.mkdtemp(prefix="mnn-publish-")
