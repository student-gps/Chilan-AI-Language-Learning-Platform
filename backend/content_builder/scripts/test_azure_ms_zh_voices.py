"""
test_azure_ms_zh_voices.py — Azure 中马来混合旁白声色试听

运行方式：
    cd backend/content_builder
    python scripts/test_azure_ms_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_ms_zh/
"""
...
DEFAULT_MS_VOICES = [
    "ms-MY-OsmanNeural",   # 男声
    "ms-MY-YasminNeural",  # 女声
]

DEFAULT_ZH_VOICE = "zh-CN-YunjianNeural"

SAMPLES = [
    "Hari ini kita belajar [zh:你好], yang bermaksud helo dalam bahasa Mandarin.",
    "[zh:马上] bermaksud segera atau tidak lama lagi. Mari dengar sekali lagi: [zh:马上].",
    "Corak [zh:不但...而且...] digunakan untuk menghubungkan dua ayat. Dalam bahasa Melayu bermaksud bukan sahaja, malah juga.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_ms_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "ms-MY"
...
def synthesize_combo(endpoint: str, api_key: str, ms_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{ms_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = ms_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Malay+Chinese mixed TTS voice samples.")
    parser.add_argument("--ms-voices", help="Comma-separated Azure Malay voices.")
    ...
    ms_voices = [v.strip() for v in (args.ms_voices.split(",") if args.ms_voices else DEFAULT_MS_VOICES) if v.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"马来语 voices: {', '.join(ms_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes, failures = [], []
    for index, ms_voice in enumerate(ms_voices, start=1):
        print(f"\n[{index:02d}] MS={ms_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, ms_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, ms_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, ms_voice, str(exc)))
        if index < len(ms_voices) and args.delay > 0:
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, ms_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  MS={ms_voice}")
    if failures:
        print("\n失败:")
        for index, ms_voice, reason in failures:
            print(f"  {index:02d}. MS={ms_voice}: {reason[:160]}")
