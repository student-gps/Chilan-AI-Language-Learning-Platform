"""
test_azure_pt_zh_voices.py — Azure 中葡混合旁白声色试听

生成「葡萄牙语 voice + 中文 voice」混合 TTS 小样，方便人工试听后选择
TTS_EXPLANATION_VOICE_PT。中文 voice 默认固定为 zh-CN-YunjianNeural。

运行方式：
    cd backend/content_builder
    python scripts/test_azure_pt_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_pt_zh/
"""
...
SAMPLES = [
    "Hoje vamos aprender [zh:你好], que significa olá em chinês.",
    "[zh:马上] significa imediatamente ou em breve. Ouça mais uma vez: [zh:马上].",
    "A estrutura [zh:不但...而且...] serve para conectar duas ideias. Em português significa não só, mas também.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_pt_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "pt-BR"
...
def synthesize_combo(endpoint: str, api_key: str, pt_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{pt_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = pt_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Portuguese+Chinese mixed TTS voice samples.")
    parser.add_argument("--pt-voices", help="Comma-separated Azure Portuguese voices.")
    ...
    pt_voices = [v.strip() for v in (args.pt_voices.split(",") if args.pt_voices else DEFAULT_PT_VOICES) if v.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"葡萄牙语 voices: {', '.join(pt_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes, failures = [], []
    for index, pt_voice in enumerate(pt_voices, start=1):
        print(f"\n[{index:02d}] PT={pt_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, pt_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, pt_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, pt_voice, str(exc)))
        if index < len(pt_voices) and args.delay > 0:
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, pt_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  PT={pt_voice}")
    if failures:
        print("\n失败:")
        for index, pt_voice, reason in failures:
            print(f"  {index:02d}. PT={pt_voice}: {reason[:160]}")
