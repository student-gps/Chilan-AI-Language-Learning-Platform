"""
test_azure_ru_zh_voices.py — Azure 中俄混合旁白声色试听

运行方式：
    cd backend/content_builder
    python scripts/test_azure_ru_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_ru_zh/
"""
...
SAMPLES = [
    "Сегодня мы изучаем [zh:你好], что означает привет по-китайски.",
    "[zh:马上] означает немедленно или сейчас же. Послушайте ещё раз: [zh:马上].",
    "Конструкция [zh:不但...而且...] соединяет два предложения. По-русски это не только, но и.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_ru_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "ru-RU"
...
def synthesize_combo(endpoint: str, api_key: str, ru_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{ru_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = ru_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Russian+Chinese mixed TTS voice samples.")
    parser.add_argument("--ru-voices", help="Comma-separated Azure Russian voices.")
    ...
    ru_voices = [v.strip() for v in (args.ru_voices.split(",") if args.ru_voices else DEFAULT_RU_VOICES) if v.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"俄语 voices: {', '.join(ru_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes, failures = [], []
    for index, ru_voice in enumerate(ru_voices, start=1):
        print(f"\n[{index:02d}] RU={ru_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, ru_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, ru_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, ru_voice, str(exc)))
        if index < len(ru_voices) and args.delay > 0:
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, ru_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  RU={ru_voice}")
    if failures:
        print("\n失败:")
        for index, ru_voice, reason in failures:
            print(f"  {index:02d}. RU={ru_voice}: {reason[:160]}")
