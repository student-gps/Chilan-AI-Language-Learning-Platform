"""
test_azure_id_zh_voices.py — Azure 中印尼混合旁白声色试听

运行方式：
    cd backend/content_builder
    python scripts/test_azure_id_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_id_zh/
"""
...
SAMPLES = [
    "Hari ini kita belajar [zh:你好], yang berarti halo dalam bahasa Mandarin.",
    "[zh:马上] berarti segera atau sebentar lagi. Mari dengarkan sekali lagi: [zh:马上].",
    "Pola [zh:不但...而且...] digunakan untuk menghubungkan dua kalimat. Artinya tidak hanya, tetapi juga.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_id_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "id-ID"
...
def synthesize_combo(endpoint: str, api_key: str, id_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{id_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = id_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Indonesian+Chinese mixed TTS voice samples.")
    parser.add_argument("--id-voices", help="Comma-separated Azure Indonesian voices.")
    ...
    id_voices = [v.strip() for v in (args.id_voices.split(",") if args.id_voices else DEFAULT_ID_VOICES) if v.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"印尼语 voices: {', '.join(id_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes, failures = [], []
    for index, id_voice in enumerate(id_voices, start=1):
        print(f"\n[{index:02d}] ID={id_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, id_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, id_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, id_voice, str(exc)))
        if index < len(id_voices) and args.delay > 0:
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, id_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  ID={id_voice}")
    if failures:
        print("\n失败:")
        for index, id_voice, reason in failures:
            print(f"  {index:02d}. ID={id_voice}: {reason[:160]}")
