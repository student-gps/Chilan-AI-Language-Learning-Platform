"""
test_azure_th_zh_voices.py — Azure 中泰混合旁白声色试听

生成「泰语 voice + 中文 voice」混合 TTS 小样，方便人工试听后选择
TTS_EXPLANATION_VOICE_TH。中文 voice 默认固定为 zh-CN-YunjianNeural。

运行方式：
    cd backend/content_builder
    python scripts/test_azure_th_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_th_zh/
"""
...
SAMPLES = [
    "วันนี้เราจะเรียน [zh:你好] ซึ่งแปลว่า สวัสดี ในภาษาจีน",
    "[zh:马上] แปลว่า ทันที หรือ เดี๋ยวนี้ ลองฟังอีกครั้ง [zh:马上].",
    "โครงสร้าง [zh:不但...而且...] ใช้เชื่อมสองประโยค ในภาษาไทยหมายความว่า ไม่เพียงแต่ แต่ยัง.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_th_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "th-TH"
...
def synthesize_combo(endpoint: str, api_key: str, th_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{th_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = th_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Thai+Chinese mixed TTS voice samples.")
    parser.add_argument("--th-voices", help="Comma-separated Azure Thai voices.")
    ...
    th_voices = [v.strip() for v in (args.th_voices.split(",") if args.th_voices else DEFAULT_TH_VOICES) if v.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"泰语 voices: {', '.join(th_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes: list[tuple[int, str, Path]] = []
    failures: list[tuple[int, str, str]] = []

    for index, th_voice in enumerate(th_voices, start=1):
        print(f"\n[{index:02d}] TH={th_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, th_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, th_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, th_voice, str(exc)))
        if index < len(th_voices) and args.delay > 0:
            print(f"  ⏸️ 等待 {args.delay:g}s，避免 Azure 限流...")
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, th_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  TH={th_voice}  ZH={args.zh_voice}")

    if failures:
        print("\n失败组合:")
        for index, th_voice, reason in failures:
            print(f"  {index:02d}. TH={th_voice}: {reason[:160]}")
