"""
test_azure_vi_zh_voices.py — Azure 中越混合旁白声色试听

生成「越南语 voice + 中文 voice」混合 TTS 小样，方便人工试听后选择
TTS_EXPLANATION_VOICE_VI。中文 voice 默认固定为 zh-CN-YunjianNeural。

运行方式：
    cd backend/content_builder
    python scripts/test_azure_vi_zh_voices.py

输出目录：
    artifacts/test_tts_output/azure_vi_zh/
"""
...
SAMPLES = [
    "Hôm nay chúng ta học câu [zh:今天天气很好]. Câu này có nghĩa là hôm nay thời tiết rất đẹp.",
    "[zh:马上] có nghĩa là ngay lập tức hoặc sắp sửa. Hãy nghe chậm một lần: [zh:马上].",
    "Cấu trúc [zh:不但...而且...] dùng để nối hai ý. Trong tiếng Việt, nó có nghĩa là không những, mà còn.",
]

OUTPUT_DIR = CONTENT_BUILDER_DIR / "artifacts" / "test_tts_output" / "azure_vi_zh"
...
def _azure_call(endpoint: str, api_key: str, voice: str, content: str, output_path: Path) -> None:
    lang_code = "-".join(voice.split("-")[:2]) if "-" in voice else "vi-VN"
    ...
    for attempt in range(1, 5):
        ...
        if resp.ok and len(resp.content) >= 100:
            ...
            return
        if resp.ok:
            reason = f"Azure returned empty/corrupt audio (bytes={len(resp.content)}, content-type={resp.headers.get('Content-Type', '')})"
        else:
            reason = f"Azure TTS error {resp.status_code}: {resp.text[:500]}"
...
def synthesize_combo(endpoint: str, api_key: str, vi_voice: str, zh_voice: str, index: int) -> Path:
    output_file = OUTPUT_DIR / f"{index:02d}_{vi_voice.replace('-', '_')}__{zh_voice.replace('-', '_')}.mp3"
        ...
        for sample_index, sample in enumerate(SAMPLES, start=1):
            for chunk in re.split(r"(\[zh:[^\]]+\])", sample):
                ...
                if match:
                    ...
                    voice, content = vi_voice, chunk.strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Azure Vietnamese+Chinese mixed TTS voice samples.")
    parser.add_argument("--vi-voices", help="Comma-separated Azure Vietnamese voices.")
    ...
    vi_voices = [item.strip() for item in (args.vi_voices.split(",") if args.vi_voices else DEFAULT_VI_VOICES) if item.strip()]
    endpoint = f"https://{region}.tts.speech.microsoft.com/cognitiveservices/v1"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"输出目录: {OUTPUT_DIR}")
    print(f"越南语 voices: {', '.join(vi_voices)}")
    print(f"中文 voice: {args.zh_voice}")

    successes: list[tuple[int, str, Path]] = []
    failures: list[tuple[int, str, str]] = []

    for index, vi_voice in enumerate(vi_voices, start=1):
        print(f"\n[{index:02d}] VI={vi_voice} | ZH={args.zh_voice}")
        try:
            output = synthesize_combo(endpoint, api_key, vi_voice, args.zh_voice, index)
            print(f"  ✅ {output.name} ({output.stat().st_size / 1024:.1f} KB)")
            successes.append((index, vi_voice, output))
        except Exception as exc:
            print(f"  ❌ {exc}")
            failures.append((index, vi_voice, str(exc)))
        if index < len(vi_voices) and args.delay > 0:
            print(f"  ⏸️ 等待 {args.delay:g}s，避免 Azure 限流...")
            time.sleep(args.delay)

    print("\n试听清单:")
    for index, vi_voice, output in successes:
        print(f"  {index:02d}. {output.name}  |  VI={vi_voice}  ZH={args.zh_voice}")

    if failures:
        print("\n失败组合:")
        for index, vi_voice, reason in failures:
            print(f"  {index:02d}. VI={vi_voice}: {reason[:160]}")
