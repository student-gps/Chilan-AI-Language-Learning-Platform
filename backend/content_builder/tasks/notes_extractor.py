...
class Task1BTextbookNotesExtractor:
    ...
    def _build_discovery_prompt(self, lesson_id: int, course_id: int) -> str:
        ...
        【重要】所有汉字内容必须统一输出简体中文，如遇繁体字请主动转换为简体。
        ...
        【不要提取】课文对话正文、生词表、Language Practice、Lesson Warm Up、Culture Notes 下的内容。
    ...
    def _build_language_notes_prompt(self, lesson_id: int, course_id: int, titles: list) -> str:
        ...
        - 【简繁去重规则 — 极其重要】本教材同时提供简体和繁体，旧册每例句连续两行（简体在前），新册整段简体后接整段繁体（可能在下一页）。无论哪种格式，只提取简体，完全忽略繁体，不重复提取。判断方法：简体字笔画更少（如"学"vs"學"）。
    ...
    def _build_grammar_section_prompt(self, title: str, grammar_id: str) -> str:
        ...
        - 【简繁去重规则 — 极其重要】本教材同时提供简体和繁体，旧册每例句连续两行（简体在前，如"昨天晚上我看了一个电影。"后紧跟"昨天晚上我看了一個電影。"），新册整段简体后接整段繁体（可能在下一页）。无论哪种格式，只提取简体，完全忽略繁体，不重复提取。判断标准：两段/两行意思完全相同、仅汉字写法不同（如"学"vs"學"），则只保留简体版本。
